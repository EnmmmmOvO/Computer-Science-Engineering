...
import dungeonmania.Entities.RareCollectableEntities.TheOneRing;
import dungeonmania.MapGenerator.MapGenerator;
import dungeonmania.exceptions.InvalidActionException;
import dungeonmania.util.Direction;
import dungeonmania.util.Position;

public class Assassin extends MovingEntities {
    ...
    public Assassin(int x, int y) {
        super(x, y, 2, "assassin", "assassin" + LocalDateTime.now().toString().replaceAll("-", ""), true);
    }
    ...
    public void update(MapGenerator mapGenerator, Direction direction) {
        ...
        while (!crrP.equals(enemy.getPosition())) {
            preP = crrP;
            crrP = pred.get(crrP);
        }
    ...
    public void interact(MapGenerator curMap) {
        ...
        Entity ring = null;
        for (Entity findring : inventorys) {
            if (findring instanceof TheOneRing) {
                ring = findring;

            }
        }
        for (Entity remains : inventorys) {
            if (remains instanceof Treasure && ring != null) {
                curMap.removeInventoryList(remains);
                curMap.removeInventoryList(ring);
                super.setIsInteractable(false);
                curMap.removeEnemyList(this);
                this.ally = true;
                return;
            } else if (remains instanceof SunStone && ring != null) {
                super.setIsInteractable(false);
                curMap.removeInventoryList(ring);
                curMap.removeEnemyList(this);
                this.ally = true;
        ...
        throw new InvalidActionException("Unsufficient item to bribe/mind-control a Assassin");
    }
}
