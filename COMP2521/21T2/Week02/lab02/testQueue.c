...
static void testQueue4(void) {
	Queue q = QueueNew();
	for(int loopA = 1; loopA <= 15; loopA ++) {
		QueueEnqueue(q , loopA);
		assert(QueueFront(q) == 1);
	}

	for (int loopB = 1; loopB <= 8; loopB ++)
		assert(QueueDequeue(q) == loopB);
	assert (QueueFront(q) == 9);
	assert (QueueSize(q) == 7);

	for (int loopC = 16, loopD = 9; 
		loopD <= 15; loopC ++, loopD ++) {
		QueueEnqueue(q, loopC);
		assert(QueueDequeue(q) == loopD);
	}
	assert(QueueFront(q) == 16);
	QueueFree(q);
}
