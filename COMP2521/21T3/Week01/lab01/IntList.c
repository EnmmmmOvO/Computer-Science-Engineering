...
void IntListInsertInOrder(IntList l, int v) {
	...
	//create a node of insert value
	struct IntListNode *n = newIntListNode(v);

	//when the list is a empty list
	if (l->first == NULL) {
		l->first = n;
		l->last = n;
		l->size++;
	...
	//when the insert value is smaller than the least value in the list;
	if (l->first->data >= v) {
		n->next = l->first;
		l->first = n;
		l->size++;
	...
	//when the insert value is larger than the largest value in the list;
	if (l->last->data <= v) {
		l->last->next = n;
		l->last = n;
		l->size++;
	...
	//copy the first address of the link list
	struct IntListNode *f;
	//find the correct place of the value which need to insert 
...
IntList IntListSortedCopy(IntList l) {
	...
	assert(l != NULL);
	// create a new List to exist the sorted list
	IntList new = IntListNew();	
	//when the list is empty, return a empty list
	if (l->first == NULL) return new;
	//copy the link list's first address
	struct IntListNode *f;
	//use the IntListInsertInorder funcion to put the element into new list in order
	for (f = l->first; f != NULL; f = f->next) {
		IntListInsertInOrder(new, f->data);
	}
	return new;
}
