...
Graph GraphMST(Graph g) {
    ...
    Graph new = GraphNew(g->nV);
    PQ pq = PQNew();
    for (int loopA = 0; loopA < g->nV; loopA++) {
        for (int loopB = loopA + 1; loopB < g->nV; loopB++) {
            if (g->edges[loopA][loopB] <= 0) continue;
            Edge e;
            e.v = loopA;
            e.w = loopB;
            e.weight = g->edges[loopA][loopB];
            PQInsert(pq, e);
        }
    }

    while (new->nE < new->nV - 1 && !PQIsEmpty(pq)) {
        Edge e = PQExtract(pq);
        GraphInsertEdge(new, e);
        if (GraphHasCycle(new)) GraphRemoveEdge(new, e.v, e.w);
    }
